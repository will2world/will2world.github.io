...
## 이미지 크기 조절하기
이미지 크기 조절은 마크다운 코드에서 조절하는 방식[^1]을 적용해보았으나 github pages에서 이를 처리하지 못하는 것을 시행착오를 통해 알 수 있었다. 구글링을 통해 알아낸 방법에 의하면 다음처럼 이미지 크기를 조절하는 것이 가능하다.
```markdown
![내 사진](/assets/images/my_picture.jpg){: width="300px" height="300px"}
```
위 코드에선 2차원(width와 height) 모두 입력을 해주었지만 둘 중 하나만 입력해도 정상적으로 작동한다.(이 경우엔 원본 이미지의 비율을 유지된다.) 단위 또한 `px`이 아니라 `%`[^2]를 사용하더라도 문제없이 작동한다. 이를 적용하여 다음처럼 이미지 크기를 조절할 수 있다.
```markdown
...
[^2]:이때 `%`는 글이 입력되는 부분에서 이미지가 차지하는 비율이 아니라 블로그 사이트 전체 중 이미지가 차지하는 비율을 의미한다는 점에 유의해야 한다.