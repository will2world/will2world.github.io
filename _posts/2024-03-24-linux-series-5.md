---
title: \[리눅스 정착기 시리즈\] 5. 프로그램 설치하기 - flatpak
toc: true
toc_sticky: true
categories:
- linux_series
...
정리하자면, 리눅스 진영에는 `수많은 종류의 distribution들`이 존재하고 이들 배포판들은 저마다 다른 구동환경을 가지고 있다. 
따라서 개발자 입장에서 이들 모두에 대해 앱을 관리하는 것은 여간 까다로운 일이 아니다.
이를 해결하고자 `컨테이너 기술`을 사용하여 앱들을 동일한 구동환경에서 실행될 수 있도록 포장하고 사용자들에게 배포하는 서비스가 flatpak이다.
...
Flathub을 통해 원하는 앱을 찾을 수 있으며 설치를 위한 터미널 명령어를 확인할 수 있다.

# flatpak을 설치하고 사용하는 방법
## flatpak 설치하기
flatpak은 현재 사용하고 있는 배포판에 따라 조금씩 차이가 있다.
그래서 각 배포판별 설치 방법에 따라 설치해야 한다.
각 배포판에 대한 설치 방법은 [Flatpak setup 페이지](https://www.flatpak.org/setup/)에서 원하는 배포판을 선택함으로써 확인할 수 있다.
<p align='center'>
    <img src="https://onedrive.live.com/embed?resid=CFD7A556C71EAC85%211985&authkey=%21AEyqZyxzwqm5K6Q&width=775&height=870" width="775" height="870" />
</p>

데비안을 사용하는 경우 데비안 마크를 클릭하면 다음과 같은 설치 방법을 만날 수 있다.
<p align='center'>
    <img src="https://onedrive.live.com/embed?resid=CFD7A556C71EAC85%211983&authkey=%21AJv96MyXEZKV_Us&width=1156&height=1119" width="auto" height="auto" />
</p>
...
특정 앱이 file system에 접근할 수 있도록 하는 명령어는 다음과 같다. 다음의 사례에서는 obsidian이 sdc2라는 usb에의 접근 권한을 부여하고 있다.
```bash
