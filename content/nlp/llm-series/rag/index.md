...
* Retrieval 과정에서 단순한 키워드 기반 유사도 사용 시 비슷하지 않는 문장을 검색한다. 예를 들어 "바다의 온도 상승이 해양 생태계에 미치는 영향은?" 이라는 질문을 했을 때, 키워드는  "온도", "해양"이 겹치지만 문맥적 의미를 고려하지 않은 "해양 온도 데이터 수집 장비의 유지보수 가이드"가 검색 되어 나온다.

* 새로운 문서를 저장할 외부 데이터베이스가 필요하다.

* 직접적인 모델 파라미터 업데이트가 아니기 때문에 지식이 모델에 '내재화'되지 않고 외부에서 '조회'되는 수준이다.(일시적인 방법)

* 여러 문서들이 retrieval되면 Generation 단계에서 조합해야할 context의 길이가 불필요하게 커지고, 모델이 핵심 근거를 구분하지 못한다.

이러한 문제들을 해결하여 LLM을 유지할 수 있는 방법을 다음 포스터에서 연재하도록 하겠습니다.
