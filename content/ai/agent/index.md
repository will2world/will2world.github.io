...
LLM agent 또는 Agentic LLM 은 chat gpt의 말을 인용하자면 "환경과 상호작용한다" 라고 하지만 쉽게 말해 "상황을 파악하여 필요한 행동을 통해 목표를 달성한다" 이다. 과거의 Input에 대한 Output을 출력하는 단순한 Model이 아닌, Model에 정보가 없으면 Model의 판단 하에 '새 문서를 찾아온다', '현재 문서와 이어진 문서를 더 찾아본다'등의 Action을 취하는 것이다.
